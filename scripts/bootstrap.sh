...
check_workflow_overlay() {
  ...
  local step_id
  if ! overlay_list="$(specify workflow overlay list speckit 2>&1)"; then
  ...
  if ! resolved_workflow="$(specify workflow resolve speckit 2>&1)"; then
...
check_initialized_state() {
  ...
  command -v specify >/dev/null 2>&1 || fail \
    "Spec Kit CLI 'specify' is not installed; run: bash scripts/bootstrap.sh"
  command -v specdd >/dev/null 2>&1 || fail \
    "SpecDD CLI 'specdd' is not installed; run: bash scripts/bootstrap.sh"
