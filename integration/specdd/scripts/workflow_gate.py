...
from boundary_builder import build_change_boundary, write_boundary  # noqa: E402
...
from validation_cli import main as validation_main  # noqa: E402
from validation_tasks import extract_repository_targets, parse_tasks_file  # noqa: E402
...
def _active_feature(root: Path) -> tuple[str, Path, str]:
    ...
    if not configured:
        ...
        try:
            ...
            raise WorkflowGateError("Spec Kit active feature state is invalid JSON") from exc
        configured = state.get("feature_directory") if isinstance(state, dict) else None

    if not isinstance(configured, str) or not configured.strip():
        raise WorkflowGateError("Spec Kit active feature directory is not configured")

    try:
        target = normalize_target(root, configured)
    except BoundaryError as exc:
        raise WorkflowGateError(f"Active feature directory is invalid: {exc}") from exc
...
def _plan_targets(root: Path, feature_dir: Path, feature_path: str) -> list[str]:
...
def _task_targets(root: Path, task_path: Path) -> list[str]:
    try:
        ...
        raise WorkflowGateError(f"Spec Kit task file was not found: {task_path}") from exc
...
def _refresh_boundary(
    ...
    *,
    ...
    value = build_change_boundary(root, targets, feature=feature, schema=schema)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except (BoundaryError, WorkflowGateError, OSError) as exc:
