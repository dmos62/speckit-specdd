...
def _metadata_path(root: Path, relative: Path, runner: RunGit) -> Path:
    ...
    base = git_dir if git_dir.is_absolute() else root / git_dir
    return base.resolve(strict=False) / relative
...
def authorization_git_baseline_path(
    root: Path,
    *,
    runner: RunGit = subprocess.run,
) -> Path:
...
def _git_head(root: Path, runner: RunGit) -> str | None:
    ...
    detail = " ".join((result.stderr or result.stdout or "").split())
    raise VerificationError(
        "Could not determine the authorization-time Git HEAD"
        + (f": {detail}" if detail else "")
    )
...
def _path_state(root: Path, path: str) -> str:
    ...
    if absolute.is_symlink():
        ...
        raise VerificationError(
            f"Git change path is not a file or symlink: {path}"
        )
...
def _git_changes(root: Path, runner: RunGit) -> list[GitChange]:
    ...
    for record in result.stdout.split("\0"):
        ...
        try:
            ...
            raise VerificationError(
                f"Git returned an invalid repository path: {raw}: {exc}"
            ) from exc
        changes.append(
            GitChange(
                target.path,
                _status_name(record[:2]),
                not os.path.lexists(target.absolute_path),
            )
        )
...
def _spec_plan_document(
    ...
    if not isinstance(feature, str) or not feature:
        raise VerificationError(
            "Authorization boundary has no feature identifier"
        )
    return {
        "schemaVersion": 1,
        "feature": feature,
        ...
        "controlTargets": [
            ...
            for path, source in sorted(controls.items())
        ],
    }


def _git_baseline_document(
    root: Path,
    boundary: Mapping[str, Any],
    runner: RunGit,
) -> dict[str, Any]:
    ...
    return {
        ...
        "entries": [
            {
                "path": item.path,
                "state": _path_state(root, item.path),
            }
            for item in _git_changes(root, runner)
        ],
    }
...
def _matches_boundary(
    ...
    if (
        value.get("feature") != boundary.get("feature")
        or value.get("boundarySha256") != _boundary_fingerprint(boundary)
    ):
        raise VerificationError(
            f"{label} does not match the boundary snapshot"
        )
...
def load_authorization_plan(
    ...
    if (
        ...
        raise VerificationError(
            "Authorization specification plan targets must be a unique "
            "string array"
        )
    ...
    for raw in raw_targets:
        if not raw.lower().endswith(".sdd"):
            raise VerificationError(
                "Authorization specification plan target is not an .sdd "
                f"file: {raw}"
            )
        try:
            ...
            raise VerificationError(
                "Authorization specification plan target is invalid: "
                f"{raw}: {exc}"
            ) from exc
        if normalized != raw:
            raise VerificationError(
                "Authorization specification plan target is not canonical: "
                + raw
            )
        targets.append(raw)
    ...
    for item in raw_controls:
        ...
        if source not in CONTROL_SELECTION_SOURCES:
            raise VerificationError(
                "Authorization control selection has invalid source: "
                f"{source}"
            )
        if path in controls:
            raise VerificationError(
...
def load_authorization_git_baseline(
    root: Path,
    path: Path,
    boundary: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if head is not None and (not isinstance(head, str) or not head):
        raise VerificationError(
            "Authorization Git baseline HEAD is invalid"
        )
    if not isinstance(raw_entries, list):
        raise VerificationError(
            "Authorization Git baseline entries must be an array"
        )
    ...
    for item in raw_entries:
        if not isinstance(item, Mapping):
            raise VerificationError(
                "Authorization Git baseline entry must be an object"
            )
        path = item.get("path")
        state = item.get("state")
        if not isinstance(path, str) or not isinstance(state, str):
            raise VerificationError(
                "Authorization Git baseline entry is invalid"
            )
        try:
            normalized = normalize_target(root, path).path
        except BoundaryError as exc:
            raise VerificationError(
                f"Authorization Git baseline path is invalid: {path}: {exc}"
            ) from exc
        if (
            normalized != path
            or path in entries
            or (
                state != "missing"
                and not state.startswith(("file:", "symlink:"))
            )
        ):
            raise VerificationError(
                f"Authorization Git baseline entry is invalid: {path}"
            )
        entries[path] = state
...
def collect_git_changes(
    ...
    *,
    ...
    try:
        feature_path = (
            normalize_target(root, str(feature_dir)).path
    ...
    if baseline and _git_head(root, runner) != baseline.get("head"):
        raise VerificationError(
            "Git HEAD changed after authorization; the operation baseline "
            "cannot be compared safely. Start a fresh authorization from "
    ...
    for item in current:
        if (
            item.path in baseline_entries
            and _path_state(root, item.path) == baseline_entries[item.path]
        ):
